...
double max_height_=1.5 , min_height_=0.0;   // @ filter range in Z axis

//  Traversability match with intensity (from pointcloud)
float INTENSITY_TRAVERSABILITY_UNKNOWN = 8.0;
float INTENSITY_TRAVERSABILITY_SAND = 6.0;
float INTENSITY_TRAVERSABILITY_GROUND_INCLINED = 5.0;
float INTENSITY_TRAVERSABILITY_STONES = 4.0;
float INTENSITY_TRAVERSABILITY_UNEVEN_GRASS = 3.0;
float INTENSITY_TRAVERSABILITY_GRASS = 2.0;
float INTENSITY_TRAVERSABILITY_ASPHALT = 1.0;
float INTENSITY_TRAVERSABILITY_FREE = 0.0;
double layer_height_step=0.1; //0.30; //to test, change traversability after each height step
...
void pc_in_cbk(const sensor_msgs::PointCloud2::ConstPtr& cloud){
  ...
  // for (  ; iter_pcIn != iter_pcIn_end ; ++iter_z , ++iter_intensity , iter_pcIn += cloud->point_step, iter_pcOut += cloud->point_step){  // @ filter range in Z axis
    // if (  ((*iter_z) >= min_height_) && ((*iter_z) <= max_height_)     )    {   // @ filter range in Z axis

  for (  ; iter_pcIn != iter_pcIn_end ; ++iter_z , ++iter_intensity , iter_pcIn += cloud->point_step, iter_pcOut += cloud->point_step){  
    if (  (*iter_z) <= max_height_     )    {   

      std::copy(iter_pcIn, iter_pcIn + cloud->point_step, iter_pcOut); // Exact copy from pcIn to pcOut

      if (  ((*iter_z) >  min_height_ )  &&  ((*iter_z) <= (min_height_ + 1*layer_height_step))      )    {   
        *iter_intensity = INTENSITY_TRAVERSABILITY_FREE;  // CUSTOMIZED INTENSITY VALUES
      }
      else if (  ((*iter_z) > (min_height_ + 1*layer_height_step))  &&  ((*iter_z) <= (min_height_ + 2*layer_height_step))      )    {   
        *iter_intensity = INTENSITY_TRAVERSABILITY_ASPHALT;  
      }
      else if (  ((*iter_z) > (min_height_ + 2*layer_height_step))  &&  ((*iter_z) <= (min_height_ + 3*layer_height_step))      )    {   
        *iter_intensity = INTENSITY_TRAVERSABILITY_GRASS;  
      }
      else if (  ((*iter_z) > (min_height_ + 3*layer_height_step))  &&  ((*iter_z) <= (min_height_ + 4*layer_height_step))      )    {   
        *iter_intensity = INTENSITY_TRAVERSABILITY_UNEVEN_GRASS;  
      }
      else if (  ((*iter_z) > (min_height_ + 4*layer_height_step))  &&  ((*iter_z) <= (min_height_ + 5*layer_height_step))      )    {   
        *iter_intensity = INTENSITY_TRAVERSABILITY_STONES;  
      }
      else if (  ((*iter_z) > (min_height_ + 5*layer_height_step))  &&  ((*iter_z) <= (min_height_ + 6*layer_height_step))      )    {   
        *iter_intensity = INTENSITY_TRAVERSABILITY_GROUND_INCLINED;  
      }
      else if (  ((*iter_z) > (min_height_ + 6*layer_height_step))  &&  ((*iter_z) <= (min_height_ + 7*layer_height_step))      )    {   
        *iter_intensity = INTENSITY_TRAVERSABILITY_SAND;  
      }
      else {   
        *iter_intensity = INTENSITY_TRAVERSABILITY_UNKNOWN;  
      }
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber sub = n.subscribe("/ouster/points", 1000, &pc_in_cbk);
