...
class TraversabilityNode:
    def __init__(self):
        ...
        self.lidar_sub = rospy.Subscriber("/ouster/points", PointCloud2, self.lidar_callback, queue_size = 1)
        # self.image_sub = rospy.Subscriber("/segmented/image", Image, self.image_callback, queue_size = 1)
        self.image_sub = rospy.Subscriber("/segmented/image/compressed", CompressedImage,  self.image_callback, queue_size=1, buff_size=16777216)


        # self.img_pub = rospy.Publisher('/encoded/image', Image, queue_size=10)
        self.camera_info_sub = rospy.Subscriber("/camera_array/cam1Right/camera_info", CameraInfo, self.camera_info_callback, queue_size = 1)
        self.pcl_pub = rospy.Publisher("/traversable_lidar", PointCloud2, queue_size=1)
        self.pc_traversable_frame_id ="traversable_sensor"
    ...
    def camera_info_callback(self, camera_info_msg):
        """
        Callback for CameraInfo message.
    ...
    def lidar_callback(self, lidar_msg):
        ...
        header.frame_id = self.pc_traversable_frame_id 
